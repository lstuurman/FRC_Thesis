...
def setup_frc(D,g_type):
    ...
    if g_type == 'ER':
        p = ER_p_value(600,.25)
        g = nx.erdos_renyi_graph(600,p)#7000
        g = fruchterman_reingold(g,20)
    elif g_type == 'BA':
        g = nx.barabasi_albert_graph(500,4)#700
        g = fruchterman_reingold(g,20)
    elif g_type == 'WS':
        k = WS_K_value(1500,.25)
        p = 0.027825594022071243
        g = nx.watts_strogatz_graph(1500,k,p)#1000
        g = fruchterman_reingold(g,20)
    elif g_type == 'PW':
        m = M_power_cluster(1000,.25)
        p = 0.666666666666666
        g = nx.powerlaw_cluster_graph(1000,m,p)# 700
        g = fruchterman_reingold(g,20)
    elif g_type == 'GM':
        r = 20/dim # 20microns
    ...
    if g_type == 'GM' or g_type == 'WS':
        ...
        cube = thicckk[384:-384,384:-384,384:-384]
