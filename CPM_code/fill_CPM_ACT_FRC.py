...
def setup(dens):
    ...
    simulation.set_constraints(cell_type = 2, lambda_act = 3500, max_act = 160) # 2500, max_act = 42
    # adhesion ; 
...
def run_grid_point(density):
    ...
    try:
        os.mkdir("../data/increase_DENS_ACT_FRC/150V_DENS_FRC2_"  +str(density))
    except:
        pass
    ...
    for i,track in enumerate(cell_track):
        ...
        fname = "150V_DENS_FRC2_"  +str(density) + "/CELL_" + str(i) + "iter" + str(iter)
        np.savetxt('../data/increase_DENS_ACT_FRC/'+fname+'.txt',track)
